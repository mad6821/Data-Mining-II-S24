...
df.c <- df %>%
  mutate(
    new_oindex = case_when(
      e_polcomp == 1 & EventViolence == 1 ~ 1, # No competition & EV = high 
      e_polcomp == 2 & EventViolence == 1 ~ 0.9, # Restricted comp & EV = high
      e_polcomp == 3 & EventViolence == 1 ~ 0.8, # Repressed comp & EV
      e_polcomp >= 1 & e_polcomp <= 3 & EventViolence == 0 ~ 0.7, # Restrictied/no competition & no EV 
      
      e_polcomp == 4 & EventViolence == 1 ~ 0.6, # Uninstitutionalized comp & EV = moderate
      e_polcomp == 4 & EventViolence == 0 ~ 0.5, # Uninstitutionalized comp & EV = moderate
      
      e_polcomp >= 5 & e_polcomp <= 8 & EventViolence == 1 ~ 0.4, # Factional comp & EV = moderate
      e_polcomp >= 5 & e_polcomp <= 8 & EventViolence == 0 ~ 0.3, # Factional comp & no EV = moderate
      
      e_polcomp == 9 & EventViolence == 1 ~ 0.2, # Institutionalized comp & EV = low
      e_polcomp == 9 & EventViolence == 0 ~ 0.1, # Institutionalized comp & no EV = low
...
###### PCA Analysis ######
fa <- factanal(df.fin, factors=1, scores="regression", rotation="none")
...
# Correlation Plot
corrMat <- cor(df.fin$fa, df.fin, use="complete.obs")
colnames(corrMat) <- c("Little/Meng Index", "Polyarchy", "New Index", "Factor Scores")
rownames(corrMat) <- c("Dimension 1")
...
# Scatterplot
ggplot(data = df.fin) +
  geom_point(aes(x = fa, y = oindex, colour = "#BACDB0"), size = 3) +
  geom_point(aes(x = fa, y = new_oindex, colour = "#729B79"), size = 3) +
  geom_point(aes(x = fa, y = v2x_polyarchy, color = "darkgreen"), size = 3, alpha=0.2) +
  labs(y="Democracy Measures", x="PCA Factor Scores", title="Raw Measures and Factor Scores")+
  scale_colour_manual(name = 'Measures', 
                      values =c('#BACDB0'='#BACDB0', '#729B79'='#729B79','darkgreen'='darkgreen'), 
                      labels = c('Little/Meng','New Index', "Polyarchy")) + 
  theme_minimal()
